...
async function loadDetail() {
    if (!urlId) { window.location.href = 'index.php'; return; }
    ...
    try {
        ...
        if (item.error) { window.location.href = 'index.php'; return; }
        ...
        document.getElementById('detail-sub').textContent   = `Cadastrado por ${item.cadastrado_por || 'Anônimo'}`;

        const nota = parseFloat(item.nota_media) || 0;
        document.getElementById('big-rating').textContent   = nota ? nota.toFixed(1) : '—';
        document.getElementById('big-stars').textContent    = nota ? CineRate.starsHTML(nota) : '';
        document.getElementById('review-count').textContent = `${item.total_avaliacoes} avaliação(ões)`;

        document.getElementById('synopsis').textContent = item.sinopse || 'Sem sinopse disponível.';
        ...
        if (session.logged && session.admin) {
            ...
            document.getElementById('btn-delete-titulo').addEventListener('click', async () => {
                if (!confirm('Remover este título permanentemente?')) return;
                const res = await fetch(`php/titulos_api.php?id=${urlId}`, { method: 'DELETE' });
                const d   = await res.json();
                if (d.success) { CineRate.toast('Título removido.', 'success'); setTimeout(() => window.location.href = 'index.php', 1000); }
        ...
        if (session.logged) {
            ...
            if (myReview) {
                const radio = document.querySelector(`input[name="nota"][value="${myReview.nota}"]`);
                if (radio) radio.checked = true;
                document.getElementById('comentario').value = myReview.comentario;
                document.getElementById('btn-review').textContent = 'Atualizar Avaliação';
            }
        }
...
function renderComments(avaliacoes, session) {
    ...
    if (!avaliacoes || !avaliacoes.length) {
        area.innerHTML = '<p style="color:var(--text-muted)">Nenhuma avaliação ainda. Seja o primeiro!</p>';
        return;
    }
    area.innerHTML = avaliacoes.map(a => `
        <div class="comment-card" id="comment-${a.id}">
            <div class="comment-header">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    ${session.admin || (session.logged && a.usuario_id == session.id)
    ...
    area.querySelectorAll('.btn-delete-comment').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const res = await fetch(`php/avaliacoes_api.php?id=${btn.dataset.id}`, { method: 'DELETE' });
            const d   = await res.json();
            if (d.success) {
