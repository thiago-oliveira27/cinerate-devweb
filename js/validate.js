const Validate = {
  ...
  clearAll(form) {
    ...
    form
      .querySelectorAll(".form-control-cr")
  ...
  tituloForm(form) {
    ...
    if (
      isNaN(anoNum) ||
      anoNum < 1888 ||
      anoNum > new Date().getFullYear() + 2
    ) {
      this.showError(
        ano,
        `Informe um ano válido (1888–${new Date().getFullYear() + 2}).`,
      );
      valid = false;
    }
  ...
  avaliacaoForm(form) {
    ...
    if (!nota) {
      if (notaErr) {
        notaErr.textContent = "Selecione uma nota.";
        notaErr.classList.add("show");
      }
      valid = false;
    }
    if (!this.minLen(comentario.value, 5)) {
      this.showError(
        comentario,
        "Escreva um comentário (mínimo 5 caracteres).",
      );
      valid = false;
    }
    return valid;
