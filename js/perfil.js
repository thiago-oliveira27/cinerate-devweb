...
async function loadPerfil() {
    ...
    document.getElementById('avatar-initials').textContent = user.nome?.charAt(0).toUpperCase();
    if (user.admin) {
        document.getElementById('profile-badge').innerHTML = '<span class="card-badge" style="color:var(--accent);border-color:var(--accent)">Admin</span>';
    }
...
async function loadMyReviews() {
    ...
    // AJAX: fetch user's reviews via a custom endpoint
    const res   = await fetch('php/minhas_avaliacoes_api.php');
    const items = await res.json();

    if (!items.length) {
        area.innerHTML = '<p style="color:var(--text-muted)">Você ainda não fez nenhuma avaliação.</p>';
        return;
    }
...
document.getElementById('perfil-form').addEventListener('submit', async (e) => {
    ...
    // Custom validation (password optional here)
    let valid = true;
    ...
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('avatar-initials').textContent = nome.value.charAt(0).toUpperCase();
